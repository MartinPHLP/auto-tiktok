...
class Word:
    def __init__(self, word, color=None, size=None):
        self.word = word
        self.color = color
        self.size = size
        self.characters = []
...
def create_text_chars(
    ...
    for i, item in enumerate(text):
        if isinstance(item, Word):
            ...
            current_size = item.size if item.size is not None else fontsize
        ...
        for char in chars:
            clip = create_text(char.text, current_size, char.color or color, font, bg_color, blur_radius, opacity, stroke_color, stroke_width)
            clips.append(clip)
